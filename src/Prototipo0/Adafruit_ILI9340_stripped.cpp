...
Adafruit_ILI9340_stripped::Adafruit_ILI9340_stripped(uint8_t cs, uint8_t dc, uint8_t mosi,
...
Adafruit_ILI9340_stripped::Adafruit_ILI9340_stripped(uint8_t cs, uint8_t dc, uint8_t rst) {
  _width    = ILI9340_TFTWIDTH;
  _height   = ILI9340_TFTHEIGHT;
  rotation  = 0;
  cursor_y  = cursor_x    = 0;
...
void Adafruit_ILI9340_stripped::drawPixel(int16_t x, int16_t y, uint16_t color) {
...
// fill a rectangle
void Adafruit_ILI9340_stripped::fillRect(int16_t x, int16_t y, int16_t w, int16_t h,
  uint16_t color) {
...
void Adafruit_ILI9340_stripped::fillScreen(uint16_t color) {
  fillRect(0, 0,  _width, _height, color);
}

unsigned char Adafruit_ILI9340_stripped::drawChar(int16_t x, int16_t y, unsigned char c,
...
void Adafruit_ILI9340_stripped::setCursor(int16_t x, int16_t y) {
  cursor_x = x;
  cursor_y = y;
}

void Adafruit_ILI9340_stripped::setTextColor(uint16_t c) {
  // For 'transparent' background, we'll set the bg 
  // to the same as fg instead of using a flag
  textcolor = textbgcolor = c;
}

void Adafruit_ILI9340_stripped::setTextColor(uint16_t c, uint16_t b) {
  textcolor   = c;
  textbgcolor = b; 
}

void Adafruit_ILI9340_stripped::setTextSize(uint8_t s) {
  textsize = (s > 0) ? s : 1;
}

void Adafruit_ILI9340_stripped::setTextWrap(boolean w) {
  wrap = w;
}
// End draw functions
/*
// Implementation Specific draw functions
void Adafruit_ILI9340_stripped::pushColor(uint16_t color) {
  //digitalWrite(_dc, HIGH);
  SET_BIT(dcport, dcpinmask);
  //digitalWrite(_cs, LOW);
  CLEAR_BIT(csport, cspinmask);
...
void Adafruit_ILI9340_stripped::invertDisplay(boolean i) {
  writecommand(i ? ILI9340_INVON : ILI9340_INVOFF);
}

// Pass 8-bit (each) R,G,B, get back 16-bit packed color
uint16_t Adafruit_ILI9340_stripped::Color565(uint8_t r, uint8_t g, uint8_t b) {
  return ((r & 0xF8) << 8) | ((g & 0xFC) << 3) | (b >> 3);
}
*/

void Adafruit_ILI9340_stripped::begin(void) {
  pinMode(_rst, OUTPUT);
...
void Adafruit_ILI9340_stripped::setAddrWindow(uint16_t x0, uint16_t y0, uint16_t x1,
 uint16_t y1) {
...
/*
void Adafruit_ILI9340_stripped::setRotation(uint8_t m) {
...
// SPI Helpers
void Adafruit_ILI9340_stripped::spiwrite(uint8_t c) {
  SPDR = c;
  while(!(SPSR & _BV(SPIF)));
}

void Adafruit_ILI9340_stripped::writecommand(uint8_t c) {
...
void Adafruit_ILI9340_stripped::writedata(uint8_t c) {
  SET_BIT(dcport,  dcpinmask);
  //digitalWrite(_dc, HIGH);
  CLEAR_BIT(clkport, clkpinmask);
  //digitalWrite(_sclk, LOW);
...
/*
void Adafruit_ILI9340_stripped::commandList(uint8_t *addr) {
...
uint8_t Adafruit_ILI9340_stripped::spiread(void) {
  SPDR = 0x00;
  while(!(SPSR & _BV(SPIF)));
  return SPDR;
}

size_t Adafruit_ILI9340_stripped::write(uint8_t c) {
...
int16_t Adafruit_ILI9340_stripped::width(void) {
  return _width;
}
 
int16_t Adafruit_ILI9340_stripped::height(void) {
  return _height;
}
/*

 uint8_t Adafruit_ILI9340_stripped::readdata(void) {
   digitalWrite(_dc, HIGH);
   digitalWrite(_cs, LOW);
   uint8_t r = spiread();
   digitalWrite(_cs, HIGH);
 ...
 uint8_t Adafruit_ILI9340_stripped::readcommand8(uint8_t c) {
   digitalWrite(_dc, LOW);
   digitalWrite(_sclk, LOW);
   digitalWrite(_cs, LOW);
   spiwrite(c);
 ...
 uint16_t Adafruit_ILI9340_stripped::readcommand16(uint8_t c) {
 digitalWrite(_dc, LOW);
 if (_cs)
 digitalWrite(_cs, LOW);
 ...
 uint32_t Adafruit_ILI9340_stripped::readcommand32(uint8_t c) {
 digitalWrite(_dc, LOW);
 if (_cs)
 digitalWrite(_cs, LOW);
 spiwrite(c);
